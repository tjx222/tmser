...
public class GifWriteWaySpider {
	...
	static class GifWriteWayProcessor implements PageProcessor{
		private Site site = Site.me().setCharset("UTF-8").setRetryTimes(3).setSleepTime(100)
				.setUserAgent("Mozilla/5.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.108 Safari/537.36");
	    @Override
	    public void process(Page page) {
	    	if(page.getUrl().regex("/zici/").match()){
	    		//http-equiv="refresh" content="0; url=/hanzi/7188.shtml"
	    		List<Selectable> nodes = page.getHtml()
		        		.xpath("//img[@id='word_bishun']").nodes();
	    	 for(Selectable meta : nodes){
	    		...
	    		if(page.getStatusCode() == 200){
			    	...
			    	logger.error("can't found mp3 ,pinyinid: "+page.getRequest().getExtra("pinyinId") +",shengdiao :"+ page.getRequest().getExtra("shengdiao"));
			    }
	    	}
	    }
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		 ...
		 Word model = new Word();
		 model.buildCondition(" and (usedLevel =:level1 or usedLevel = :level2 )")
		 .put("level1", 1).put("level2", 2);
