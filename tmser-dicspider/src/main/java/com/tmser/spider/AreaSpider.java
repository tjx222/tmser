...
public class AreaSpider {
	...
	static class AreaPageProcessor implements PageProcessor{
	    ...
	    void addreq(Area p,List<Area> allChild,Page page){
	    	if(allChild.size() > 0){
    			...
    			if(p.getParentId() == 1 || p.getParentId()==32 || p.getParentId()==34 || p.getParentId()==33  || p.getParentId() == 26 || p.getParentId() == 8 || p.getParentId() == 31){
    				return;
    			}
	...
	public static void main(String[] args) {
		 ...
		 Spider.create(new AreaPageProcessor(areaDao))
		 .addPipeline(new ConsolePipeline())
		 .addPipeline(pipeline)
		 .setDownloader(httpClientDownloader)
		 .addUrl("https://jisuarea.market.alicloudapi.com/area/query?parentid=0")
		 .thread(1).run();
	}
