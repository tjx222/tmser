...
public class PaginationInterceptor implements Interceptor {
    ...
    public boolean willDoQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) throws SQLException {
        ...
        if (page == null || page.getSize() < 0 || !page.searchCount()) {
            if(page != null && page.getSize() > 0 && page.getSize() < Long.MAX_VALUE && !page.searchCount()){
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, BoundSql boundSql) throws SQLException {
        IPage<?> page = ParameterUtils.findPage(parameter).orElse(null);
        if (null == page) {
            return;
        }
    ...
    protected String autoCountSql(boolean optimizeCountSql, String sql) {
        ...
        try {
            ...
            // 无法优化使用原 SQL
            logger.warn("optimize this sql to a count sql has exception, sql: \"{}\" , exception:\n {}" , sql, e.getCause());
        } catch (Exception e) {
            logger.warn("optimize this sql to a count sql has error, sql:\"{}\", exception:\n{}" , sql, e);
        }
        return lowLevelCountSql(sql);
    }
    ...
    protected void handlerLimit(IPage<?> page) {
        ...
        if(!page.searchCount() && limit < Long.MAX_VALUE){ //不分页多查询一条
            limit = limit + 1;
        }
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (target instanceof Executor) {
            ...
            if (!willDoQuery(executor, ms, parameter, rowBounds, resultHandler, boundSql)) {
                return Collections.emptyList();
            }
            beforeQuery(executor, ms, parameter, boundSql);
            return handleResult(invocation.proceed(), parameter);
        }
        return invocation.proceed();
    }

    private Object handleResult(Object proceed, Object parameter) {
      if(proceed instanceof List){
          ...
          if(!page.searchCount() && CollectionUtils.isNotEmpty((List)proceed)){
              List<?> records = (List<?>) proceed;
              page.setSize(page.getSize() -1);
              page.setTotal(records.size() + Math.max((page.getCurrent() - 1) * page.getSize(), 0L));
