...
public class Money implements Serializable, Comparable<Money> {
    ...
    private static final long serialVersionUID = -8027451345311721769L;
     ...
     * 缺省的币种代码，为CNY（人民币）。
     */
    public static final transient String DEFAULT_CURRENCY_CODE = "CNY";

    /**
     * 缺省的取整模式，为<code>BigDecimal.ROUND_HALF_EVEN
     * （四舍五入，当小数为0.5时，则取最近的偶数）。
     */
    public static final transient int DEFAULT_ROUNDING_MODE = BigDecimal.ROUND_HALF_EVEN;
     ...
     */
    private static final transient int[] centFactors = new int[]{1, 10, 100,
            1000};
     ...
     * @return
     */
    public boolean greaterEquals(Money other) {
        return compareTo(other) >= 0;
    }
     ...
     * @return
     */
    public boolean isGreaterEqualsZero() {
        return greaterEquals(ZERO);
    }
     ...
     * @return
     */
    public boolean isLessEqualsZero() {
        return compareTo(ZERO) <= 0;
    }
    // 货币算术 ==========================================
    ...
    /**
     * 货币累加,返回自身，链式调用，提高高性能的累加。在大量统计时使用，平时可以使用不加后缀的add方法，其它类似。
     * <p>
     * <p>
    ...
    /**
     * 货币累减。返回自身，链式调用，提高高性能的累加。在大量统计时使用，平时可以使用不加后缀的add方法，其它类似。
     * <p>
     * <p>
    ...
    /**
     * 货币累乘。返回自身，链式调用，提高高性能的累加。在大量统计时使用，平时可以使用不加后缀的add方法，其它类似。
     * <p>
     * <p>
    ...
    /**
     * 货币累除。返回自身，链式调用，提高高性能的累加。在大量统计时使用，平时可以使用不加后缀的add方法，其它类似。
     * <p>
     * <p>
    ...
    /**
     * 依元为单位进行拆成targets份
     * 除不尽向上入1角 最后一份最少如100元分3份 结果为[34,34,32]
     *
     * @param targets
     * @return
     */
    public Money[] allocateByYuan(int targets) {
        Money[] results = new Money[targets];
        long highResult = -1;
        if (cent % (targets * 100) == 0) {
            highResult = cent / targets;
        } else {
            highResult = cent / targets / 100 * 100 + 100;
        }
        long remainder = cent;
        for (int i = 0; i < targets; i++) {
            if (remainder < 0) {
                results[i] = new Money(0);
            } else if (remainder > highResult) {
                results[i] = new Money(highResult);
            } else {
                results[i] = new Money(remainder);
            }
            remainder = remainder - highResult;
        }
        return results;
    }
    ...
    /**
     * 依角为单位进行拆成targets份
     * 除不尽向上入1角 最后一份最少如100元分3份 结果为[33.4,33.2,33.2]
     *
     * @param targets
     * @return
     */
    public Money[] allocateByJiao(int targets) {
        Money[] results = new Money[targets];
        long highResult = -1;
        if (cent % (targets * 10) == 0) {
            highResult = cent / targets;
        } else {
            highResult = cent / targets / 10 * 10 + 10;
        }
        long remainder = cent;
        for (int i = 0; i < targets; i++) {
            if (remainder < 0) {
    ...
    /**
     * 保留到元 舍去分 如79.61 就当79
     */
    public void roundByYuan() {
        setAmount(getAmount().setScale(0, BigDecimal.ROUND_DOWN));
    }

    /**
     * 保留到角 舍去分 如79.61 就当79.6
     */
    public void roundByJiao() {
        setAmount(getAmount().setScale(1, BigDecimal.ROUND_DOWN));
    }
