...
import com.tmser.blog.model.dto.TagWithPostCountDTO;
...
import com.tmser.blog.repository.PostTagRepository;
...
public class PostTagServiceImpl extends AbstractCrudService<PostTag, Integer>
    ...
    public List<TagWithPostCountDTO> listTagWithCountDtos(Sort sort) {
        ...
        QueryWrapper<Tag> objectQueryWrapper = new QueryWrapper<>();
        sort.stream().forEach(sortItem -> objectQueryWrapper.orderBy(true, sortItem.getDirection() == Sort.Direction.ASC, sortItem.getProperty()));
        // Find all tags
        List<Tag> tags = tagRepository.selectList(objectQueryWrapper);
    ...
    @Override
    public Page<Post> pagePostsBy(Integer tagId, Page pageable) {
        Assert.notNull(tagId, "Tag id must not be null");
    ...
    @Override
    public Page<Post> pagePostsBy(Integer tagId, PostStatus status, Page pageable) {
        Assert.notNull(tagId, "Tag id must not be null");
        Assert.notNull(status, "Post status must not be null");
        Assert.notNull(pageable, "Page info must not be null");
    ...
    public List<PostTag> mergeOrCreateByIfAbsent(Integer postId, Set<Integer> tagIds) {
        ...
        // Remove post tags
        postTagRepository.deleteBatchIds(postTagsToRemove.stream().map(PostTag::getId).collect(Collectors.toList()));
    ...
    public List<PostTag> removeByPostId(Integer postId) {
        Assert.notNull(postId, "Post id must not be null");
        List<PostTag> allByPostIds = postTagRepository.findAllByPostId(postId);
    ...
    public List<PostTag> removeByTagId(Integer tagId) {
        Assert.notNull(tagId, "Tag id must not be null");
        List<PostTag> allByTagIds = postTagRepository.findAllByTagId(tagId);
