...
import com.tmser.blog.exception.*;
...
import com.tmser.blog.theme.ThemePropertyScanner;
...
import static com.tmser.blog.model.properties.PrimaryProperties.THEME;
import static com.tmser.blog.model.support.HaloConst.DEFAULT_THEME_ID;
import static com.tmser.blog.utils.FileUtils.copyFolder;
import static com.tmser.blog.utils.FileUtils.deleteFolderQuietly;
import static com.tmser.blog.utils.VersionUtil.compareVersion;
...
public class ThemeRepositoryImpl
    ...
    public void setActivatedTheme(@NonNull String themeId) {
        ...
        final Option newThemeOption = optionRepository.findByKey(THEME.getValue())
                .map(themeOption -> {
                    // set theme id
                    themeOption.setOptionValue(themeId);
                    return themeOption;
                })
                .orElseGet(() -> new Option(THEME.getValue(), themeId));
        optionRepository.insert(newThemeOption);
    ...
    public ThemeProperty attemptToAdd(ThemeProperty newProperty) {
        // 1. check existence
        final boolean alreadyExist = fetchThemePropertyByThemeId(newProperty.getId()).isPresent();
        if (alreadyExist) {
        ...
        final Path sourceThemePath = Paths.get(newProperty.getThemePath());
        ...
        return ThemePropertyScanner.INSTANCE.fetchThemeProperty(targetThemePath)
